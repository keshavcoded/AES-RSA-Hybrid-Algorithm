...
def main():
    ch = int(input('What do you want to do?\n\n1.Embed data into image\n2.Extract data from image\n\nInput(1/2): '))
    if ch == 1:
        ip_file = input('\nEnter cover image name(path)(with extension): ')
        #text = input('Enter data to encode: ')
        
        #seq = (Hybridenc.EncryptHybrid.ciphertext,Hybridenc.EncryptHybrid.cipherkey,Hybridenc.EncryptHybrid.DS)
        q = Hybridenc.EncryptHybrid.ciphertext
        s = Hybridenc.EncryptHybrid.cipherkey
        d = Hybridenc.EncryptHybrid.DS
        #text = a.join(seq)
        str1 = ' '
        str2 = ' '
        for ele in s:
        ...
        text = 'Cipher text : '+str(q)+'\n'+'Encrypted Key : '+'['+str1+']'+'\n'+'Digital Signature : '+'['+str2+']'

        print(' ')

        print('Embed Message is ready to be encrypted into the cover image')
        print(' ')
        ...
        try:
            ...
            print(" ")
            print(colored('Encrypted in image Successfully!','yellow'))
            print(colored('Image Data Loss = {:.5f}%'.format(loss),'yellow'))
    elif ch == 2:
        ip_file = input('Enter image(path)(with extension): ')
        pwd = input('Enter password: ')
        try:
            ...
            d = Hybridenc.EncryptHybrid.DS
            print('Cipher text  :')
            print(colored(q,'white'))
            print('Encrypted Symmetric Key  :')
            print(colored(s,'white'))
            print('Digital Signature  :')
            print(colored(d,'white'))
            print(colored('Decrypted from image Successfully!','yellow'))
    else:
        print('Wrong Choice!')
